...
def popraw(r):
    ...
    for i in range(len(m)-2):
        if m[i] == "/" and m[i+1] == "(":
            ...
            while m[ko] != ")":
                if m[ko] == "x" or m[ko] == "y":
                    tekstbox.insert(END, "NIPOPRAWNE RÓWNANIE - niewiadoma nie może być w mianowniku\n")
                    tekstbox.see("end")
                    return 0
                ko += 1
    if m.find("()", 0, len(m)) != -1 or m.find("-)", 0, len(m)) != -1 or m.find("+)", 0, len(m)) != -1 or m.find("/)", 0, len(m)) != -1 or m.find("*)", 0, len(m)) != -1:
        tekstbox.insert(END, "NIPOPRAWNE RÓWNANIE - niepoprawne użycie nawiasów\n")
        tekstbox.see("end")
        return 0
    if m.find("**", 0, len(m)) != -1 or m.find("//", 0, len(m)) != -1 or m.find("*+", 0, len(m)) != -1 or m.find("+*", 0, len(m)) != -1:
        tekstbox.insert(END, "NIPOPRAWNE RÓWNANIE - niepoprawne użycie działań\n")
        tekstbox.see("end")
        return 0
    if m.find("-*", 0, len(m)) != -1 or m.find("-/", 0, len(m)) != -1 or m.find("*-", 0, len(m)) != -1 or m.find("+/", 0, len(m)) != -1:
        tekstbox.insert(END, "NIPOPRAWNE RÓWNANIE - niepoprawne użycie działań\n")
...
def spr_inne_bledy(r):
    ...
    if c2 - c1 != c3 -c2 or d2 - d1 != d3 - d2:
        tekstbox.insert(END, "Podane równanie nie jest liniowe (I-ego stopnia z 2-ma niewiadomymi)\n")
        tekstbox.see("end")
        return 0
    return r
...
window.title("ROZWIĄZYWANIE UKlADÓW RÓWNAn")
